...
class OngModel
{
    ...
    function editar($dados)
    {
        ...
        if (!$stmt->execute()) {
            return false; // se algo falhar
        }

        return $stmt->rowCount();
    }

    public function salvarImagemOng($ongId, $caminho)
    {
        ...
        $this->atualizarImagem($ongId, $idImagem);
    ...
    public function buscarCaminhoImagem($imagemId)
{
    ...
    public function buscarImagemId($id)
    {
        $query = "SELECT imagem_id FROM {$this->tabela} WHERE ong_id = :id";
        $stmt = $this->pdo->prepare($query);

...
class ImagemModel
{
    ...
    public function deletarImagem(int $idImagem)
    {
        
        $sql = "SELECT caminho FROM imagens WHERE imagem_id = :id";
        $stmt = $this->pdo->prepare($sql);
        $stmt->bindValue(':id', $idImagem);
        $stmt->execute();
        $imagem = $stmt->fetch(PDO::FETCH_ASSOC);

        if ($imagem && file_exists(__DIR__ . '/../../../' . $imagem['caminho'])) {
            unlink(__DIR__ . '/../../../' . $imagem['caminho']); // Apaga o arquivo físico
        }

        // Exclui o registro do banco
        $sql = "DELETE FROM imagens WHERE imagem_id = :id";
        $stmt = $this->pdo->prepare($sql);
        $stmt->bindValue(':id', $idImagem);
        return $stmt->execute();
    }

...
function proximo(indice) {
    ...
    if (indice == 1) {
        ...
        if (
            !validarCampo(input[12], 13, 'Digite a agência corretamente.', 6) ||
            !validarCampo(input[13], 14, 'Escolha o tipo de Conta.') ||
            !validarCampo(input[14], 15, 'Digite a conta corretamente.', 8) ||
            !validarCampo(input[15], 16, 'Digite o nome completo do titular.', 8)
        ) {
            check[4].classList.remove('input-valid');
            return false;
        }
        check[4].classList.add('input-valid');
        moverPara(indice, 16);
    }
    else if (indice == 6) {
        if (
